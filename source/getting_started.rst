...
Our Watchman software handles most use-cases that involve extracting license plate data from an IP camera stream.
...
The Watchman Agent installs on your local hardware and will use as many CPU resources as you provide.
Memory and disk requirements are relatively constant.
Once OpenALPR initializes, the process uses roughly 500MB per analysis thread (CPU core), and that memory usage will not increase.
...
After completing installation, you can can choose between four data destination options depending on how you wish to use the Watchman Agent
  ...
  1. **OpenALPR Cloud:** Send data to https://cloud.openalpr.com (a Watchman webserver hosted by OpenALPR)
  2. **Watchman Webserver:** Send data to an on-premise Watchman webserver (hosted by you on a Linux machine)
  3. **Generic HTTP URL:** Send data to your own HTTP endpoint
...
More details can be found in :ref:`application_integration`.
...
For hardware without a GUI
  ...
  .. code-block:: bash
    ...
    # Linux
    alprlink-register -w https://[ip_address_of_web_server]
...
**4) Manually edit configuration files**
  ...
  5. Restart the Agent
    ...
    * *Terminal:* :code:`sudo systemctl restart openalpr-daemon`